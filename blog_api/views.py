...
from blog.models import Card, Board, UpdateData, Equipe, Colaborador, Epi, BoardEpi
...
from .serializers import (CardSerializer, BoardSerializer, UpdateSerializer, 
                          BoardUpdateSerializer, EquipeSerializer, ColaboradorSerializer,
                          ColabEquipesSerializer, EpiSerializer, BoardEpiSerializer)
...
class EpiData(generics.ListAPIView):
    """A dummy docstring."""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get_queryset(self):
        ...
        print(item)
        print('uowww')
        #print(Epi.objects.filter(board=item))
        return Epi.objects.filter(card=item)
        # return Epi.objects.all()
    
class BoardEpiData(generics.ListAPIView):
    """A dummy docstring."""
    permission_classes = [permissions.IsAuthenticated]
    serializer_class = BoardEpiSerializer

    def get_queryset(self):
        """A dummy docstring."""
       
        return BoardEpi.objects.all()
